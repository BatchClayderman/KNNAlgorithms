...
kNNResultsFilepath = "kNNCoordResults.txt"
...
def printHelp() -> None:
	print("Python script for KNN coord querying based on RTree. ", end = "\n\n")
	print("Option: ")
	...
	print("Format: ")
	print("\tpython kNNCoordQuerying.py [/coords|--coords|coords] coordsFilepath [/offsets|--offsets|offsets] offsetsFilepath [/rTree|--rTree|rTree] rTreeFilepath [/kNNQueries|--kNNQueries|kNNQueries] kNNQueriesFilepath [/k|-k|k] k [/o|-o|o|/output|--output|output] outputFilepath", end = "\n\n")
	print("Example: ")
	print("\tpython kNNCoordQuerying.py /coords coords.txt /offsets offsets.txt /rTree Rtree.txt /kNNQueries NNqueries.txt /k 10")
	print("\tpython kNNCoordQuerying.py /coords coords.txt /offsets offsets.txt /rTree Rtree.txt /kNNQueries NNqueries.txt /k 10 /output kNNCoordsResults.txt", end = "\n\n")
...
def handleCommandline() -> dict:
	...
	if len(argv) > 1 and len(argv) not in (3, 5, 7, 9, 11, 13):
		...
		return False
	dicts = {"coords":coordsFilepath, "offsets":offsetsFilepath, "rTree":rTreeFilepath, "kNNQueries":kNNQueriesFilepath, "k":10, "output":kNNResultsFilepath}
	pointer = None
	for arg in argv[1:]:
